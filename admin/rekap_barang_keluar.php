...
$sheet->getColumnDimension('C')->setWidth(15); // Jumlah Keluar
$sheet->getColumnDimension('D')->setWidth(25); // Tujuan
...
$nomor = 1; // Nomor urut
$totalBarangKeluar = 0; // Untuk menghitung total barang keluar
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $tanggalKeluar = date('d-m-Y', strtotime($row['TANGGALKELUAR']));
    $sheet->setCellValue('E' . $rowNum, $tanggalKeluar);
    ...
    $totalBarangKeluar += (int)$row['JUMLAHKELUAR'];
    $rowNum++;
    $nomor++;
}
...
$footerRow = $rowNum + 1;
$sheet->setCellValue('A' . $footerRow, 'TOTAL BARANG KELUAR:');
$sheet->setCellValue('C' . $footerRow, $totalBarangKeluar);
